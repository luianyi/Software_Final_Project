...
public class UITest
{
    ...
    public void TestTimeSearch()
    {
        ...
        onView(withId(R.id.search_fromDate)).perform(typeText("20200412_104401"), closeSoftKeyboard());
        //In the location search window Enter the desire search for lower bound latitude
        onView(withId(R.id.search_toDate)).perform(typeText("20200412_104601"), closeSoftKeyboard());

        //In the search window Press Search to go back to main activity
        onView(withId(R.id.button)).perform(click());

        //Verify that there is a result
        onView(withId(R.id.editTextCaption)).check(matches(withText("valid")));
    ...
    public void TestFaceRecognition()
    {
        ...
        onView(withId(R.id.editTextCaption)).check(matches(withText("FaceWithin0.1")));
    ...
    @Test
    public void TestScan() // testing the function of sharing photo to social media
    {
        try{Thread.sleep(1000);}catch(Exception e){}
